...
class FollowupPrint(models.TransientModel):
    ...
    def _get_partners_followp(self):
        ...
        for partner_id, followup_line_id, date_maturity, date, id in move_lines:
            ...
            cutoff_date = fups[followup_line_id][0]
            if date_maturity:
                if isinstance(date_maturity, str):
                    date_maturity_obj = datetime.datetime.strptime(date_maturity, '%Y-%m-%d').date()
                else:
                    date_maturity_obj = date_maturity
                if date_maturity_obj <= cutoff_date:
